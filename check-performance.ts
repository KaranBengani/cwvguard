...
export const checkPerformance = async (baseUrl: string,routes:string[] , apiKey: string, threshold = 50) => {
  ...
  for (const route of routes) {
    ...
    const extractMetric = (key:string) => parseFloat(metrics[key]?.numericValue || 0);
...
export const checkPerformancePrebuild = async (baseUrl: string, apiKey: string, threshold = 50,routes:string[]) => {
  const blocked: string[] = [];
  const results:any[] = [];
